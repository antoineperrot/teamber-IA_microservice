...
def convert_to_arrays(matrice_competence :pd.DataFrame, matrice_projet:pd.DataFrame, capacite_utilisateur:pd.DataFrame):
...
def make_arcs_and_c(tasks: pd.DataFrame, matrice_competence :np.array, matrice_projet: np.array, curseur_politique: float):
    matrice_specialisation = compute_matrice_specialisaton(matrice_competence)
    matrice_compromis = matrice_specialisation * curseur_politique + (1-curseur_politique) * matrice_competence
    ...
    for i in range(n_tasks):
        for j in range(n_people):
            if matrice_competence[competences[i],j] > 0 and  matrice_projet[projet_referent[i],j] == 1:  #si une personne est capacité de résoudre la tache (niveau compétence >= 1 et quelle est sur le projet )
...
def make_output_dataframe(solution : np.array, arcs:list):
    ...
    for j in range(len(arcs)):
        if solution[j] > 0 :
            arc = arcs[j]
            out = out.append(pd.DataFrame({'ID Task':[arc[0]],'ID User':[arc[1]],'Durée (h)':[solution[j]]}))  
