...
def mock_back_data(
    date_start: str, date_end: str, avg_n_users: int = 5, avg_n_tasks: int = 25
):
    """
    ...
    print(
        "Attention, les impératis mockées sont disponibles uniquement entre 2022-09-05 08:30:00+0000 et \
...
def mock_imperatifs(utl: int, date_start, date_fin: str):
    ...
    out = pd.DataFrame(
        {
            ...
            "evt_xdate_fin": [
                pd.Timestamp("2022-09-05 09:30:00+0000"),
    ...
    out = out.loc[
        (out["evt_xdate_fin"] >= date_start) & (out["evt_xdate_debut"] <= date_fin)
    ]
