...
def get_next_part(i_current_part: int, parts: pd.DataFrame) -> int:
    ...
    while next_part['start'] == current_part['end'] and \
            next_part['evt_spkevenement'] == current_part['evt_spkevenement'] and \
            i_next_part < len(parts):
        i_next_part = i_next_part + 1
        next_part = parts.iloc[i_next_part if i_next_part < len(parts)
                               else len(parts) - 1]
    return i_next_part


def add_event(parts: pd.DataFrame | None = None,
              i_current_part: int | None = None,
              i_next_part: int | None = None,
              events: dict[str: list] | None = None) -> dict[str: list]:
    """
    ...
    if events is None and i_current_part is None and i_next_part is None and parts is None:
        events = {'start': [],
                  "end": [],
                  "evt_spkevenement": [],
                  "evt_sfkprojet": [],
...
def move_indexes_forward(parts: pd.DataFrame, i_current_part: int | None = None, i_next_part: int | None = None)\
        -> tuple[int, int]:
    """
...
def make_events(parts: pd.DataFrame) -> pd.DataFrame:
    ...
    while i_current_part < len(parts):
        ...
        events = add_event(parts=parts, i_current_part=i_current_part, i_next_part=i_next_part, events=events)
        i_current_part, i_next_part = move_indexes_forward(parts=parts, i_current_part=i_current_part,
...
def schedule_parts(availabilities: pd.DataFrame,
                   tasks: pd.DataFrame,
    ...
    while not no_more_parts and not no_more_sections:
        ...
        current_section_start = availabilities.iloc[i_current_section]['timestamp_debut']
    ...
    scheduled_tasks_parts = scheduled_tasks_parts[['start', 'end', 'evt_spkevenement', 'evt_sfkprojet', 'priorite']]
    return scheduled_tasks_parts


def schedule_events(availabilities: pd.DataFrame,
                    tasks: pd.DataFrame,
                    ordonnancement: Ordonnancement):
    """
