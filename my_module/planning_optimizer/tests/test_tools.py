...
def test_calcul_base_planning():
    import pandas as pd
    from data_test.reference_data.reference_variables import ref_plagehoraire_path, DATE_DEBUT, DATE_FIN,LONGUEUR_MIN,ref_base_path, read_base_or_imperatifs
    
    from module.planning_optimizer.tools import calcul_base_planning
    ph_df = pd.read_json(ref_plagehoraire_path)
    output = calcul_base_planning(ph_df, DATE_DEBUT, DATE_FIN, LONGUEUR_MIN=LONGUEUR_MIN)
...
def test_add_imperatifs():
    ...
    from data_test.reference_data.reference_variables import ref_imperatifs_path,ref_base_path, ref_base_sans_imperatifs_path,LONGUEUR_MIN, read_base_or_imperatifs
...
def test_split_tasks():
    import pandas as pd
    from data_test.reference_data.reference_variables import ref_tasks_path, mod_length, ref_splitted_tasks_path
