...
def handler_clean_hor(hor: pd.DataFrame) -> pd.DataFrame:
    hor.sort_values(by=['eeh_sfkperiode', 'eeh_xheuredebut', 'eeh_xheurefin'], inplace=True)
...
def handler_union_hor(union: pd.DataFrame) -> pd.DataFrame:
    ...
    while len(temp) > 0:
        ...
        to_delete = temp.loc[(temp['eeh_sfkperiode'] == day) & (temp['eeh_xheuredebut'] >= heure_debut) \
                             & (temp['eeh_xheurefin'] <= heure_fin)]
        if len(to_delete) > 0:
            temp = temp.drop(index=to_delete.index)
            temp.reset_index(inplace=True, drop=True)

        kept = temp.loc[(temp['eeh_sfkperiode'] == day) & (temp['eeh_xheuredebut'] <= heure_fin) \
                        & (temp['eeh_xheuredebut'] >= heure_debut) & (temp['eeh_xheurefin'] >= heure_fin)]
