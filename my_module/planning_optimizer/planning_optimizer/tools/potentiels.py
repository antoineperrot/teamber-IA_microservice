...
def V_TempsPerdu(taches, base_planning_array):
    ...
    prop_temps_perdu = (
        temps_perdu / temps_total_dispo
    )  # proportion de temps perdu par rapport au temps total dispo
...
def V_Priorites(taches):
    ...
    return (
        np.linalg.norm(target - prio, ord=1) / len(np.unique(prio)) / len(prio)
    )  # (prio != target).sum()/len(prio)
