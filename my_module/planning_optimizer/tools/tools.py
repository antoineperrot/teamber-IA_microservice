...
def calcul_base_planning(
    ...
    current_date = datetime.date(DATE_DEBUT.year, DATE_DEBUT.month, DATE_DEBUT.day)
    ...
    while current_datetime_fin < DATE_FIN:
        ...
        if last_jour != jour_present:
            n_days = (
                jour_present - last_jour
                if jour_present > last_jour
                else (7 - last_jour + jour_present)
            )
    ...
    df.drop(
        ["Heure début", "index", "Heure fin", "Date", "Jour", "Nom"],
        axis=1,
        inplace=True,
    )
