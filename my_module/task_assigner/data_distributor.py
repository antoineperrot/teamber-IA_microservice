...
def solve_problem(
    ...
    matrice_competence, matrice_projet, capacite_utilisateur = convert_to_arrays(
        matrice_competence, matrice_projet, capacite_utilisateur
    )

    arcs, c = make_arcs_and_c(
        tasks, matrice_competence, matrice_projet, curseur_politique
    )
    ...
    if l.status != 0:
        l = linprog(
            -c, A_eq=A, b_eq=b, method="interior-point", options={"maxiter": 1500}
        )
        if l.status != 0:
    ...
    solution = l.x
    output = {
        "solution": make_output_dataframe(solution, arcs).to_dict(),
