...
class step:
    def __init__(self, x : tuple, y: tuple, func : object, test_nature='standard'):
        self.x = x ; self.y = y;
        self.cmd = 'from ' + func.__module__ +' import '+ func.__name__ + ' as func_to_test'
        self.module = func.__module__
        self.name = func.__name__
        self.complete_name = self.module + '.'+ self.name
        
    def fx(self):
        try: exec(self.cmd, globals())
        except: raise ImportError(self.cmd)
            
        try : fx = func_to_test(*self.x)
        except: raise AssertionError(f"{self.complete_name} n'a pas pu être appliquée aux données synthétiques.")
            
        if not isinstance(fx, tuple): fx = [fx]
    ...
    def test(self):
        ...
        if len(fx) != len(self.y):
            raise AssertionError(f"Le nombre de variables en sortie de {self.complete_name} a changé. Attendu : {len(self.y)}, Obtenu : {len(fx)}")
        for output, expected_output in zip(fx, self.y):
            if isinstance(expected_output, dict) or isinstance(expected_output, list) or isinstance(expected_output, int) or isinstance(expected_output, str):
                assert output == expected_output, f"{self.complete_name} ne reproduit plus les résultats attendus."
            elif isinstance(expected_output, pd.core.frame.DataFrame):
                assert all(output == expected_output), f"{self.complete_name} ne reproduit plus les résultats attendus."
            elif isinstance(expected_output, np.ndarray):
                assert (output == expected_output).all(), f"{self.complete_name} ne reproduit plus les résultats attendus."
            else:
...
def test_task_assigner_on_synthetic_data(path_synthetic_data_test="my_module/task_assigner/synthetic_data_for_testing/"):
    for i, file in enumerate(os.listdir(path_synthetic_data_test)):
        exp = pickle.load(open(path_synthetic_data_test + file, 'rb'))
        print("\n" + '#'*30 + f" TEST DATASET {i+1}/{len(os.listdir(path_synthetic_data_test))} " + '#'*30)
        exp.test()
        print("\n")


def generate_synthetic_testing_data_task_assigner(n_exp=10,
                                    path_synthetic_data_test = "task_assigner/synthetic_data_for_testing/"):
    """
    ...
    for i in range(n_exp):
        ...
        df_rmd_prj, df_rmd_cmp, df_rmd_tsk, df_rmd_dsp = add_local_ids_in_dfs(df_prj, df_cmp, df_tsk, df_dsp,
        ...
        d_tsk_to_cmp, d_tsk_to_prj, d_tsk_to_lgt, d_utl_to_dsp = make_usefull_mapping_dicts(df_rmd_tsk, df_rmd_dsp)
        ...
        df_out = make_output_dataframe(solution_vector, arcs, cost_func,
                            d_tsk_to_lgt, d_tsk_to_cmp, d_tsk_to_prj, d_utl_to_dsp)


        steps.append(step(
        (solution_vector, arcs, cost_func,
                        d_tsk_to_lgt, d_tsk_to_cmp, d_tsk_to_prj, d_utl_to_dsp),
        (df_out,),
        ...
        df_rmd_out = remap_df_out(df_out,
        ...
        steps.append(step(
        (df_rmd_out, d_utl_to_dsp, utl_to_int,),
        (stat_utl,),
        make_stat_utl
        ))
        ...
        steps.append(step(
        (df_rmd_out, d_tsk_to_lgt, int_to_tsk,),
        (stat_tsk,),
        make_stat_tsk
        ))
